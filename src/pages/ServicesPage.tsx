...
const productPricing = {
  ...
  const handleValidate = () => {
    if (!mileage || Number(mileage) <= 0) {
      setSuccessMessage(isArabic ? '?????????: ???????????? ?????.' : 'Kilométrage requis pour valider.');
      return;
    }
  ...
  const translation = {
    ...
    searchPlaceholder: isArabic ? '??? ?????? ?? ??? ??????' : 'Téléphone ou immatriculation',
    noMatch: isArabic ? '?? ???? ????' : 'Aucun client trouvé',
    addCustomer: isArabic ? '????? ???? ????' : 'Ajouter un nouveau client',
    customerName: isArabic ? '??? ??????' : 'Nom du client',
    phone: isArabic ? '??????' : 'Téléphone',
    plate: isArabic ? '??? ??????' : 'Immatriculation',
    vehicle: isArabic ? '???????' : 'Véhicule',
    mileage: isArabic ? '?????????' : 'Kilométrage',
    oil: isArabic ? '??? ?????' : 'Type d’huile',
    recommended: isArabic ? '???? ??' : 'Recommandé',
    filters: isArabic ? '???????' : 'Filtres',
    labor: isArabic ? '????? ??????' : 'Main d’œuvre',
                    ...
                    <div key={key} className={`rounded-xl border p-3 transition ${isChecked ? 'border-slate-800 bg-slate-950/70' : 'border-slate-800 bg-slate-900/40 opacity-60'}`}>
                      <div className="mb-2 flex items-center justify-between gap-2">
                        <label className="block text-xs uppercase tracking-[0.2em] text-slate-500">
                          {key === 'oil' ? (isArabic ? '???? ?????' : 'Filtre à huile') : key === 'air' ? (isArabic ? '???? ??????' : 'Filtre à air') : key === 'fuel' ? (isArabic ? '???? ??????' : 'Filtre carburant') : (isArabic ? '???? ????????' : 'Filtre habitacle')}
                        </label>
